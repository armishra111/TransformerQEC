...
# Cell 6: POS_ENCODING config + Cell 7: model classes
exec(nb02[6])
exec(nb02[7])
print("[OK] Model classes defined")

# Cell 9: training setup functions
exec(nb02[9])
...
# ---------------------------------------------------------------------------
# Test pos_encoding variants: "joint" (original) and "separate" (space/time)
# ---------------------------------------------------------------------------
print("\n" + "=" * 60)
print("PHASE 9: Positional encoding variants (d=3)")
print("=" * 60)
...
for pos_enc in ["default", "rope"]:
    m = TransformerQEC(pos_encoding=pos_enc)
    key = jax.random.PRNGKey(0)

    # Init
    p = m.init(key, jnp.zeros((1, sl)), jnp.zeros((1,)), cd)['params']
    param_count = sum(x.size for x in jax.tree_util.tree_leaves(p))

    # Forward pass
    out = m.apply({'params': p}, jax.device_put(syn), jax.device_put(pe), cd)
    assert out.shape == (8, 2), f"pos_encoding={pos_enc}: wrong shape {out.shape}"
    assert not np.any(np.isnan(np.array(out))), f"pos_encoding={pos_enc}: NaN in output"

    # Gradient flow
    def loss_fn(params):
        logits = m.apply({'params': params}, jax.device_put(syn), jax.device_put(pe), cd)
        return cross_entropy_loss(logits, jax.device_put(lab)), logits
    ...
    assert grad_norm > 0, f"pos_encoding={pos_enc}: zero gradients"

    print(f"  pos_encoding={pos_enc:>8s}: params={param_count:,}, "
          f"loss={float(loss):.4f}, grad_norm={grad_norm:.4f} [OK]")

print("  [OK] All pos_encoding variants pass")
